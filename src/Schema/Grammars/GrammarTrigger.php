    ...
    public function compileTrigger(Blueprint $blueprint, Fluent $command): string
    {
        if (filled($command['transitionTables'])) {
            ...
            if (filled($new) || filled($old)) {
                $referencing = implode(' ', array_filter(['referencing', $new, $old], static fn (?string $part) => filled($part)));
            }
        }
        ...
        $sql = implode(' ', array_filter($index, static fn (?string $part) => filled($part)));
