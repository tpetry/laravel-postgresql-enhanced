...
class MigrationIndex
{
    ...
    public function compileCommand(Grammar $grammar, string $table, Fluent $command, string $type): string
    {
        ...
        if (\is_array($command['with'])) {
            $with = array_map(static fn (mixed $value) => match ($value) {
                true => 'on',
                false => 'off',
                default => (string) $value,
            }, $command['with']);
            $with = array_map(static fn (string $value, string $key) => "{$key} = {$value}", $with, array_keys($with));
        ...
        $columns = array_map(static function (string|ExpressionContract|ExpressionClass $column) use ($grammar): string {
        ...
        if ('fulltext' === $command['name']) {
            $columns = array_map(static function (string $column, int $index) use ($command, $grammar): string {
    ...
    public function createName(string $type, string $prefix, string $table, array $columns): string
    {
        ...
        $columns = array_map(static fn (string $column) => Str::before($column, ' '), $columns);
