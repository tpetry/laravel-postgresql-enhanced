    ...
    protected function asDateTime(mixed $value): CarbonInterface
    {
        try {
            ...
            if (version_compare(App::version(), '7.0.0', '<') && \is_string($value)) {
                $parsed = rescue(static fn () => Date::createFromFormat('Y-m-d H:i:s.u', $value), report: false);
                if ($parsed) {
                    return $parsed;
                }

                $parsed = rescue(static fn () => Date::createFromFormat('Y-m-d H:i:sP', $value), report: false);
                if ($parsed) {
                    return $parsed;
                }

                $parsed = rescue(static fn () => Date::createFromFormat('Y-m-d H:i:s', $value), report: false);
                if ($parsed) {
                    return $parsed;
                }
            }
